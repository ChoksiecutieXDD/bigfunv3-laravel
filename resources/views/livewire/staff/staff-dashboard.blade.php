<div>
    ...
    <div>
        <h2 class="text-xl font-bold text-white flex items-center gap-2 mb-4">
            <span class="material-symbols-rounded">pending_actions</span> Pendings Book
        </h2>
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-white/50">
            <div class="w-full overflow-x-auto custom-scrollbar">
                <table class="w-full text-left border-collapse min-w-[1000px]">
                    ...
                    <tbody class="divide-y divide-gray-50 text-sm">
                        ...
                        @empty
                        <tr>
                            <td colspan="9" class="p-8 text-center text-gray-500 italic">No future pending bookings.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <!-- Livewire Pagination for Pending Bookings -->
            @if ($pend_tasks->lastPage() > 1)
            <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50">
                <span class="text-xs text-gray-500 font-medium">Page {{ $pend_tasks->currentPage() }} of {{ $pend_tasks->lastPage() }}</span>
                <div class="flex gap-2">
                    <button wire:click="previousPage('pend_page')" @if($pend_tasks->onFirstPage()) disabled class="px-4 py-2 bg-gray-100 border text-gray-300 rounded-lg text-xs font-bold cursor-not-allowed" @else class="px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-lg text-xs font-bold hover:bg-gray-100 transition" @endif>Previous</button>
                    <button wire:click="nextPage('pend_page')" @if(!$pend_tasks->hasMorePages()) disabled class="px-4 py-2 bg-gray-100 border text-gray-300 rounded-lg text-xs font-bold cursor-not-allowed" @else class="px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-lg text-xs font-bold hover:bg-gray-100 transition" @endif>Next</button>
                </div>
            </div>
            @endif
        </div>
