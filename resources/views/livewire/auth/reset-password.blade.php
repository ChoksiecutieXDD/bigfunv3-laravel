<div class="min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden bg-plum-light">
    ...
    <div class="w-full max-w-md bg-white p-8 md:p-10 rounded-3xl shadow-2xl relative z-10 animate-enter">
        ...
        <form wire:submit="updatePassword" class="space-y-5">
            ...
            <div x-data="{ show: false }" class="relative group">
                ...
                <span class="material-symbols-rounded absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-plum">lock</span>

                <button type="button" @click="show = !show" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-plum transition flex items-center justify-center focus:outline-none">
            ...
            <div x-data="{ show: false }" class="relative group">
                <input :type="show ? 'text' : 'password'" wire:model="password_confirmation" placeholder="Confirm Password" required
                    class="modern-input w-full py-4 pl-14 pr-12 bg-gray-50 rounded-2xl border border-gray-100 focus:outline-none transition-all text-gray-800 font-medium focus:bg-white">
                <span class="material-symbols-rounded absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-plum">lock_reset</span>
            ...
            <button type="submit"
                class="w-full py-4 bg-plum text-white font-bold rounded-2xl text-lg hover:bg-plum-dark hover:shadow-lg hover:-translate-y-1 transition-all duration-300 mt-2 flex justify-center items-center gap-2">
