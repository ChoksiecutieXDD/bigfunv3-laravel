<x-sidebar.sidebar-layout>
    <div x-data="{ remindersOpen: false }" class="w-full max-w-[1600px] mx-auto space-y-8 pb-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-[#2D3748] dark:text-white">Calendar View</h2>
                <p class="text-gray-500 mt-1 text-sm font-medium">Overview of appointments and financials.</p>
            </div>

            <a href="{{ route('bookings.create') }}" class="hidden md:flex bg-[#9E6B73] hover:bg-[#86545C] text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-pink-900/20 items-center gap-2 transition transform hover:scale-105 active:scale-95 text-sm">
        ...
        <div class="bg-white rounded-[2rem] shadow-xl p-8">
            <h3 class="text-xl font-bold text-[#2D3748] mb-6 flex items-center justify-between">
                Monthly Financials
                <span class="text-xs bg-gray-100 text-gray-500 px-3 py-1 rounded-full">{{ \Carbon\Carbon::create()->month($currentMonth)->format('F') }} Stats</span>
            </h3>
            ...
            <div class="flex flex-col xl:flex-row gap-10">
                <div class="flex-1">
                    ...
                    <div class="grid grid-cols-3 py-4 border-b border-gray-50 text-sm items-center">
                        ...
                        <div class="text-right font-bold text-[#9E6B73]">{{ $stats['saturdayBookings'] ?? 0 }}</div>
                        <div class="text-right font-bold text-[#9E6B73]">${{ number_format($stats['saturdayRevenue'] ?? 0, 2) }}</div>
                    </div>

                    <div class="grid grid-cols-3 py-4 text-sm items-center">
                        <div class="text-left font-bold text-[#9E6B73]">Year To Date ({{ $currentYear }})</div>
        ...
        <div class="bg-white rounded-[2rem] shadow-xl flex flex-col h-[800px] overflow-hidden mt-8">
            ...
            <div class="calendar-header-bar p-5 flex flex-col justify-between shrink-0 z-30 relative gap-4">
                <div class="flex flex-col xl:flex-row justify-between items-center w-full gap-4">
                    ...
                    <div class="flex items-center gap-4 w-full md:w-auto justify-center md:justify-start">
                        ...
                        <h3 class="text-2xl font-bold tracking-tight text-center min-w-[200px]">
                    ...
                    <div class="flex items-center gap-4 w-full md:w-auto justify-center">
                        ...
                        <select wire:model.live="statusFilter" class="filter-select bg-white text-[#9E6B73] font-bold text-xs rounded-xl h-10 focus:outline-none cursor-pointer hover:bg-gray-50 transition border border-white/20 shadow-sm w-36">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar bg-white relative">
                ...
                <div wire:loading class="absolute inset-0 bg-white/70 backdrop-blur-sm z-20 flex items-center justify-center">
                    <div class="animate-spin rounded-full h-8 w-8 border-4 border-gray-200 border-t-[#9E6B73]"></div>
                </div>

                @forelse($groupedBookings as $date => $bookings)
                <div class="day-header flex justify-between items-center">
                    <span>{{ \Carbon\Carbon::parse($date)->format('l, F jS') }}</span>
                    <span class="text-xs bg-[#9E6B73] text-white px-2 py-1 rounded-full">{{ count($bookings) }}</span>
                </div>

                <div>
                    @foreach($bookings as $booking)
                    <a href="{{ route('bookings.show', $booking->id) }}" class="booking-card card-{{ $booking->color_code }}">
        ...
        <button @click="remindersOpen = true" class="fab-reminders z-30 bg-white hover:bg-gray-50 text-[#9E6B73] w-12 h-12 rounded-full border border-gray-200 flex items-center justify-center transition shadow-lg group">
        ...
        <a href="{{ route('bookings.create') }}" class="lg:hidden fixed bottom-6 right-6 w-14 h-14 bg-[#9E6B73] text-white rounded-full shadow-2xl flex items-center justify-center hover:bg-[#86545C] transition transform active:scale-90 border-4 border-white z-30">
        ...
        <div class="reminder-panel" :class="{ 'open': remindersOpen }">
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 class="font-bold text-lg text-gray-800 flex items-center gap-2">
                    <span class="material-symbols-rounded text-[#9E6B73]">notifications_active</span> Reminders
                </h3>
