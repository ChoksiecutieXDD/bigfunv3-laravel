...
<head>
    ...
    <!-- Assuming your standard Vite setup for the TALL stack -->
    @vite(['resources/css/app.css', 'resources/js/app.js'])

    <!-- Inline fallback for your app.css typography & colors (optional, just for safety) -->
    <style>
        [x-cloak] {
...
<body class="bg-[var(--color-bg-page)] text-[var(--color-text-main)] antialiased min-h-screen flex items-center justify-center selection:bg-[var(--color-plum)] selection:text-[var(--color-plum-light)]">
    ...
    <!-- Alpine Component: Handles simple hover animations & button loading state -->
    <div
        class="max-w-2xl px-6 py-16 text-center"
        x-data="{ isHovered: false, loading: false }">

        <!-- Error Code with Alpine hover effect -->
        <h1
        ...
        <div class="mt-6 flex justify-center space-x-3">
            <div class="w-3.5 h-3.5 rounded-full bg-[var(--color-bg-custom)] animate-bounce" style="animation-delay: 0s;"></div>
            <div class="w-3.5 h-3.5 rounded-full bg-[var(--color-plum)] animate-bounce" style="animation-delay: 0.15s;"></div>
            <div class="w-3.5 h-3.5 rounded-full bg-[var(--color-plum-dark)] animate-bounce" style="animation-delay: 0.3s;"></div>
        </div>

        <!-- Copy -->
        <h2 class="mt-8 text-3xl md:text-4xl font-bold tracking-tight">Oops! We lost this page.</h2>
        <p class="mt-4 text-lg font-medium text-[var(--color-text-main)] opacity-75 max-w-md mx-auto">
            The page you're looking for seems to have wandered off into the void. Let's get you back on track.
        </p>
        ...
        <div class="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a
                ...
                <!-- Alpine Loading Spinner (Triggers on click) -->
